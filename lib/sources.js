...
export class NjuskaloSource {
  ...
  blockedUntil=0;
  lastRun=null;
  constructor({fetchImpl=globalThis.fetch,maxPages=20,delayMs=1800,lookbackDays=12,now=()=>Date.now()}={}){this.fetchImpl=fetchImpl;this.maxPages=maxPages;this.delayMs=delayMs;this.lookbackDays=lookbackDays;this.now=now;}
  async fetch() {
    ...
    for(let page=1;page<=this.maxPages;page++){
      ...
      try{
        ...
        pagesScanned=page;
        const recent=pageListings.filter(item=>{
          const age=runNow-new Date(item.publishedAt).getTime();
          return Number.isFinite(age)&&age>=-DAY_MS&&age<=this.lookbackDays*DAY_MS;
        });
        for(const item of recent)found.set(item.id,item);
        if(pageListings.length===0){stoppedReason="empty-page";break;}
        if(recent.length===0){stoppedReason="older-than-lookback";break;}
      }catch(error){
    ...
    this.lastRun={pagesScanned,count:listings.length,stoppedReason,partial:/partial/.test(stoppedReason),lookbackDays:this.lookbackDays};
    if(!listings.length)throw new Error(`Njuškalo format nije prepoznat ili nema oglasa iz zadnjih ${this.lookbackDays} dana`);
    return listings;
  }
}

import { IndexOglasiSource,NekretnineHrSource,OglasnikSource } from "./additional-sources.js";
import { CroRealSource } from "./croreal-source.js";
import { KvadratkoSource } from "./kvadratko-source.js";
...
export const liveSources = [new NjuskaloSource(),new NekretnineHrSource(),new IndexOglasiSource(),new OglasnikSource(),new CroRealSource(),new KvadratkoSource()];
