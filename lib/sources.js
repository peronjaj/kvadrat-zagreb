...
export function parseNjuskaloList(html) {
  ...
  for (let index=0; index<candidates.length; index++) {
    ...
    unique.set(match[2],{
      ...
      previousPrice:null, condition:inferCondition(`${title} ${block}`), floor:inferFloor(block),
      ...
      features:[/balkon|lođa|loggia|terasa/i.test(block)&&"Balkon / terasa",/\blift\b/i.test(block)&&"Lift",/parking|garaž|parkirno/i.test(block)&&"Parking"].filter(Boolean)
    });
