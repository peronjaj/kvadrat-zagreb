...
export function floorAdjustmentPct(floor,basement=false){
  if(basement)return -18;
  if(floor<=0)return -6;
  if(floor===1)return 0;
  if(floor===2)return 1;
...
export function estimateListing(listing) {
  ...
  const floorPct=floorAdjustmentPct(listing.floor,listing.basement);
  ...
  return {
    ...listing,
    basement:Boolean(listing.basement),
    priceM2: actualM2,
    marketBaseM2:base,
