...
const isBasement=value=>/\bsuteren\b/i.test(String(value??""));
...
const recentEnough=(value,now,days=12)=>{
  const age=now-new Date(value).getTime();
  return Number.isFinite(age)&&age>=-DAY_MS&&age<=days*DAY_MS;
};

function baseListing({id,source,title,neighborhood,street,area,rooms,price,condition,floor,basement=false,elevator,parking,year,publishedAt,sourceUrl,imageUrl,index,features=[],dateEstimated=false}){
  return {id,source,title,neighborhood:neighborhood||"Zagreb",street:street||neighborhood||"Zagreb",area,rooms:rooms||2,price,previousPrice:null,condition:condition||"održavano",floor:floor||0,basement:Boolean(basement),elevator:Boolean(elevator),parking:Boolean(parking),year:year||0,publishedAt,sourceUrl,imageUrl:imageUrl||null,comparables:12,microFactor:1,imageTone:toneFor(index),features,dateEstimated};
}
...
export function parseNekretnineHrList(html,{now=Date.now(),onlyNew=true}={}){
  ...
  return results.filter(item=>!onlyNew||item?.realEstate?.isNew).map((item,index)=>{
    ...
    if(!ad.id||!area||!price||price/area<800||price/area>15000)return null;
    ...
    const floorText=property.floor?.floorOnlyValue||property.floor?.value||"";
    const basement=isBasement(`${ad.title} ${property.description||""} ${floorText}`);
    return baseListing({id:`ne-${ad.id}`,source:"Nekretnine.hr",title:ad.title,neighborhood:location.macrozone||location.microzone||"Zagreb",street:location.microzone||location.address||location.macrozone,area,rooms:number(property.rooms)||2,price,condition:inferCondition(`${ad.title} ${property.description||""}`),floor:parseFloor(floorText),basement,elevator,parking,year,publishedAt:new Date(now).toISOString(),sourceUrl:item.seo?.url||`https://www.nekretnine.hr/oglasi/${ad.id}/`,imageUrl:property.photo?.urls?.medium||property.photo?.urls?.small,index,features:[basement&&"Suteren",features.some(x=>x.type==="balcony")&&"Balkon / terasa",elevator&&"Lift",parking&&"Parking"].filter(Boolean),dateEstimated:true});
  }).filter(Boolean);
}
...
export class NekretnineHrSource{
  ...
  lastRun=null;
  constructor({fetchImpl=globalThis.fetch,maxPages=12,delayMs=700,now=()=>Date.now()}={}){this.fetchImpl=fetchImpl;this.maxPages=maxPages;this.delayMs=delayMs;this.now=now;}
  async fetch(){
    ...
    for(let page=1;page<=this.maxPages;page++){
      ...
      const items=parseNekretnineHrList(await response.text(),{now:this.now(),onlyNew:false});
      const before=found.size;items.forEach(item=>found.set(item.id,item));pagesScanned=page;
      if(!items.length||found.size===before)break;
      if(page<this.maxPages&&this.delayMs)await wait(this.delayMs);
    }
    this.lastRun={pagesScanned,count:found.size,stoppedReason:"active-inventory-backfill",dateEstimated:true};
    if(!found.size)throw new Error("Nekretnine.hr nije vratio aktivne zagrebačke stanove");
    return [...found.values()];
  }
}
...
export function parseOglasnikRsc(text,{now=Date.now(),lookbackDays=12}={}){
  ...
  for(const match of text.matchAll(/"ad":(\{[\s\S]*?\}),"isHomepage"/g)){
    ...
    if(!publishedAt||!recentEnough(publishedAt,now,lookbackDays))continue;
    ...
    const floorText=findParam(ad,"re_floor_number")?.value||"";
    ...
    const basement=isBasement(`${ad.title} ${floorText} ${extra}`);
    ...
    unique.set(ad.id,baseListing({id:`og-${ad.id}`,source:"Oglasnik.hr",title:ad.title,neighborhood:micro,street:micro,area,rooms,price,condition:inferCondition(`${ad.title} ${extra}`),floor,basement,elevator,parking,year,publishedAt,sourceUrl:`https://www.oglasnik.hr/stanovi-prodaja/${slugify(ad.title)}-oglas-${ad.id}`,imageUrl:ad.media?.[0]?.listing||ad.media?.[0]?.thumb,index:unique.size,features:[basement&&"Suteren",elevator&&"Lift",parking&&"Parking",/balkon|terasa|lođa/i.test(extra)&&"Balkon / terasa"].filter(Boolean)}));
  }
...
export class OglasnikSource{
  ...
  lastRun=null;
  constructor({fetchImpl=globalThis.fetch,maxPages=10,delayMs=700,lookbackDays=12,now=()=>Date.now()}={}){this.fetchImpl=fetchImpl;this.maxPages=maxPages;this.delayMs=delayMs;this.lookbackDays=lookbackDays;this.now=now;}
  async fetch(){
    const found=new Map();let pagesScanned=0;
    for(let page=1;page<=this.maxPages;page++){
      ...
      const items=parseOglasnikRsc(await response.text(),{now:this.now(),lookbackDays:this.lookbackDays});
      items.forEach(item=>found.set(item.id,item));pagesScanned=page;
      if(page<this.maxPages&&this.delayMs)await wait(this.delayMs);
    }
    this.lastRun={pagesScanned,count:found.size,stoppedReason:"lookback-window",lookbackDays:this.lookbackDays};
...
export function parseIndexPayload(payload,{now=Date.now(),lookbackDays=12}={}){
  ...
  return candidates.map((item,index)=>{
    ...
    const basement=isBasement(text);
    return baseListing({id:`ix-${id}`,source:"Index Oglasi",title:item.title,neighborhood,street:neighborhood,area,rooms:number(item.rooms||item.roomCount)||2,price,condition:inferCondition(text),floor:parseFloor(item.floor),basement,elevator:/"lift"\s*:\s*true/i.test(text),parking:/parking|garaž/i.test(text),year:number(item.year||item.constructionYear),publishedAt,sourceUrl:item.url?new URL(item.url,"https://www.index.hr").href:null,imageUrl:item.imageUrl||item.image?.url||null,index,features:[basement&&"Suteren"].filter(Boolean)});
  }).filter(Boolean).filter(item=>recentEnough(item.publishedAt,now,lookbackDays));
}
...
export class IndexOglasiSource{
  ...
  lastRun=null;
  constructor({fetchImpl=globalThis.fetch,lookbackDays=12,now=()=>Date.now()}={}){this.fetchImpl=fetchImpl;this.lookbackDays=lookbackDays;this.now=now;}
  async fetch(){
    try{
      ...
      const items=parseIndexPayload(await response.json(),{now:this.now(),lookbackDays:this.lookbackDays});
      if(!items.length)throw new Error("Index Oglasi format nije prepoznat");
      this.lastRun={count:items.length,pagesScanned:1,stoppedReason:"lookback-window",lookbackDays:this.lookbackDays};return items;
    }catch(error){
      const snapshot=INDEX_SNAPSHOT.filter(item=>recentEnough(item.publishedAt,this.now(),this.lookbackDays));
