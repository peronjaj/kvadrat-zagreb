...
const plausibleListing=item=>{
  const area=Number(item?.area);
  const price=Number(item?.price);
  const priceM2=price/area;
  return area>0&&price>0&&priceM2>=800&&priceM2<=15000;
};
...
export async function syncListings({
  ...
  if(fetchedLive.length){
    const normalized=deduplicate(fetchedLive.filter(plausibleListing).map(item=>({...item,neighborhood:normalizeNeighborhood(item.neighborhood)}))).map(estimateListing);
    const previousLive=(previous.listings||[]).filter(item=>!item.source?.includes("demo")&&!item.snapshot&&plausibleListing(item));
    listings=mergeAndRetain(previousLive,normalized,now);
  }else{
    const cachedLive=(previous.listings||[]).filter(item=>!item.source?.includes("demo")&&!item.snapshot&&plausibleListing(item));
  ...
  listings=listings.filter(plausibleListing).map(estimateListing);
